...
alltraining = list(csv.reader(open("training.csv","rb"), delimiter=','))
...
alltest = list(csv.reader(open("test.csv","rb"), delimiter=','))
...
print  "Loop again on test file to load the EventId, bdt score pairs"
testpairs=[]
...
i=0
for entry in alltest:
    testpairs+=[(entry[iid],entry[iscore])]
    i+=1


print "Sort on the score " # score is the second variable
testpairssorted=sorted(alltest,key=lambda entry: entry[1])

print "Build a map key: Id, value: rank"
testpairsdict={}
r=1 # kaggle asks to start at 1
for e in testpairssorted:
    testpairsdict[e[0]]=r
    r+=1


if len(testpairsdict)!=len(testpairssorted):
    print " ERROR ! Size of input file : ",len(testpairssorted), " size of map : ",len(testpairsdict), " Should be equal. Maybe identical values ? " 
    raise Exception # should not happen
...
for entry in alltest:
    ...
    rank=testpairsdict[entry[iid]]
    if rank>550000:
        print "large rank: ",rank, " for event ",i, " ",entry[id]
        raise Exception (" ")
...
# delete big objects
del alltraining,alltest,testpairs,testpairssorted,testpairsdict
